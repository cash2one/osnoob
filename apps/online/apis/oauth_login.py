...
from apps import db_config, qq, config, mdb_user
from apps.blueprint import online
from apps.shared_tools.image.image_up import img_fetch
from apps.config import Permission
from apps.online.process.user_verify import User, password_hash
...
def oauth_login_register(open_id,oauth_app,username, sex,email, ava_url=None, province="",city="",district=""):
    #　验证是否第一次
    if oauth_app == "qq":
        up = mdb_user.db.user_profile.find_one({'qq_openid':open_id})
        if up:
    ...
    # 第一次登录---------------------------------------------------------------------------
    t_uaername = username
    while mdb_user.db.user.find_one({"username":username}):
        username = "{}_{}".format(t_uaername, randint(1000,100000))

    role_id = mdb_user.db.role.find_one({"permissions":Permission.AVERABGEUSER})["_id"]
    password = ""
    for i in range(0,10):
        password = password_hash("{}{}".format(password, rndChar()))
    ...
    user_id = mdb_user.user.insert(user)
    ...
    # post type
    mdb_user.db.post_type.insert({'user_id':user.id, 'type':[]})
    # post tag
    mdb_user.db.tag.insert({'user_id':user.id, 'tag':[]})
